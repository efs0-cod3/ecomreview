  ...
  return (
    ...
    <nav className="navbar_main pt-5 pr-5 pl-5 pb-5">
      <div className="navbar_menu">
        <img src="/images/icon-menu.svg" alt="bars" className="ham mr-5 pointer" onClick={toggleMenu}/>
        <img src="/images/logo.svg" alt="logo" className="logo" />
      </div>
      <ul className={`navbar-items animate__animated ${closed ? "animate__slideOutLeft noShow-nav" : "animate__slideInLeft"}`} >

        <div className="close-menu">
        <img src="/images/icon-close.svg" alt="bars" className="ham mr-5 mt-5 mb-5 pointer" onClick={toggleMenu}/>
        </div>
        {items.map((item) => (
      ...
      <div className="navbar-utils">
        ...
        <img src="/images/icon-cart.svg" alt="cart" className="cart mr-1" />
          <span className="cartItemsLength">{`${cartItems?.length > 0 ? cartItems.length  : ''}` }</span>
          </div>
        <img
